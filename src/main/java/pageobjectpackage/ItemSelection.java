...
public class ItemSelection {
	...
	@FindBy(xpath = "//*[@id=\"txtQOH\"]")
	WebElement qoh;
	
	@FindBy(xpath = "//*[contains(@id,'_Qty')]")
	WebElement qty_in_table;
	
	@FindBy(xpath = "//*[contains(@class,'increment')]")
	WebElement btn_add_in_table;
	...
	public void item_Select_overQOH1(String ItemCode)
			throws Exception {
		
		/***** Select items over QOH count - directly while adding to the table*****/
		WebDriverWait wait = new WebDriverWait(driver, 30);
		wait.until(ExpectedConditions.visibilityOf(slt_drp_down));
		slt_drp_down.click();
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		slt_search.sendKeys(Keys.ENTER);
		String qohValue = qoh.getText();
		int qtyAboveQoh = Integer.parseInt(qohValue)+1;
		String qty = Integer.toString(qtyAboveQoh);
		inp_quantity.sendKeys(qty);
		btn_add.click();
		Thread.sleep(2000);
		try{
		...
		Thread.sleep(2000);
		String qohValuePostAdd = qoh.getText();
		Assert.assertEquals(qohValue, qohValuePostAdd);

	}
	
	public void item_Select_overQOH2(String ItemCode)
			throws Exception {
		
		/***** Select items over QOH count - adding after added in the table*****/
		WebDriverWait wait = new WebDriverWait(driver, 30);
		wait.until(ExpectedConditions.visibilityOf(slt_drp_down));
		slt_drp_down.click();
		Thread.sleep(2000);
		...
		String qohValue = qoh.getText();
		inp_quantity.clear();
		inp_quantity.sendKeys(qohValue);
		btn_add.click();
		Thread.sleep(4000);
		qty_in_table.click();
		Thread.sleep(2000);
		btn_add_in_table.click();
		Thread.sleep(2000);
		try{
		    driver.switchTo().alert();
		    // If it reaches here, it found a popup
		...
		slt_search.sendKeys(Keys.ENTER);
		String qohValuePostAdd = qoh.getText();
		Assert.assertEquals(qohValuePostAdd, qohValue);

	}
	
	public void item_Select_3(String ItemNo, String ItemCode, String ItemName, String ItemQty, String ActualAmount)
			throws Exception {
		
		/***** Select items from drop down after verifying the QOH value*****/
		WebDriverWait wait = new WebDriverWait(driver, 30);
		wait.until(ExpectedConditions.visibilityOf(slt_drp_down));
		slt_drp_down.click();
		Thread.sleep(2000);
		...
		String qohValue = qoh.getText();
		int actualQoh = Integer.parseInt(qohValue);
		int itemQty = Integer.parseInt(ItemQty);
		if(actualQoh >= itemQty )    
		{
			/****** Send the Item quantity if it is less than QOH value ********/
			inp_quantity.sendKeys(ItemQty);
		}
		else if (actualQoh == 0)
		{
			/****** Send error message and exit if QOH is equals to Zero as it can't be added to table *********/
			throw new SkipException("Item can't be selected as QOH value is Zero");
			
		}
		else
		{
			
			/****** Send the Item quantity if given quantity is greater than QOH value.
			 *  Hence, passing random number between 1 and QOH value ********/
			Random r = new Random();
			int lowerBound = 1;
			int upperBound = actualQoh;
			int result = r.nextInt(upperBound-lowerBound) + lowerBound;
			ItemQty = Integer.toString(result);   //To store modified Item Quantity for verification
			inp_quantity.sendKeys(ItemQty);
			String expectedrate = actualrat.getAttribute("value");
			int rate = Integer.parseInt(expectedrate);  //To modify the actual amount received from TestData.xlsx
			int Amount = rate * result;
			ActualAmount = Integer.toString(Amount);
